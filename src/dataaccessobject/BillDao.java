...
public class BillDao {
    ...
    public static ArrayList<Bill> getAllRecordsByInc(String date) {
        ...
        try {
            ResultSet rs = DataBaseOperations.getData("select * from bill where data like '%" + date + "%'");
            while (rs.next()) {
                Bill bill = new Bill();
    ...
    public static ArrayList<Bill> getAllRecordsByDesc(String date) {
        ArrayList<Bill> arrayList = new ArrayList<>();
        try {
            ResultSet rs = DataBaseOperations.getData("select * from bill where data like '%" + date + "%' order By id DESC");
            while (rs.next()) {
                Bill bill = new Bill();
                bill.setId(rs.getInt("id"));
                bill.setName(rs.getString("name"));
